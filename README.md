...
1.настраивает GDT
2.принимает параметры grub
3.инициализирует framebuffer и запускает fbcon драйвер
4.инициализирует аллокатор
5.инициализирует ramdisk и монтирует его файловую систему
6.настраивает IDT
7.запускает драйвер PS/2 клавиатуры
8.запускает модули прописанные в конфиге
9.запускает минимальную консоль для работы с системой(не готово)
...
СТРУКТУРЫ ЯДРА ОПИСАНЫ В ФАЙЛЕ zhirtypes.h. регистрация экспортов в kernel.c
...
модули - это доверенный код. нет ограничения безопасности для них. для ограничения безопасности пользовательского ПО используется zhir-vm или unixbox
